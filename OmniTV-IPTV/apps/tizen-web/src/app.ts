...
export function createApp(root: HTMLElement) {
  ...
  const secureStorage = new TizenSecureStorage();
  const parental = new ParentalManager({ storage: secureStorage });
  ...
  function setCredentials(creds: XtreamCredentials | null) {
    state.credentials = creds;
  }

  function render(screen: ScreenName, params?: Record<string, unknown>) {
    root.innerHTML = '';
