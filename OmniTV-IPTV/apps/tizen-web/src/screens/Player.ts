...
export function createPlayerScreen(context: ScreenContext, params: PlayerParams) {
  ...
  container.innerHTML = `
    <video controls autoplay style="width:100%;height:100%;background:#000"></video>
  `;
  ...
  if (!client) {
    context.showToast('Sem credenciais');
  ...
  if (video.canPlayType('application/vnd.apple.mpegurl')) {
    ...
    hls.on(Hls.Events.ERROR, (_, data) => {
      if (data.fatal) {
        context.showToast(`Erro de reprodução: ${data.type}`);
      }
    });
  } else {
    context.showToast('Player HLS não suportado');
  }
